...
values <- data.frame(id=seq_along(mean_val), mean=mean_val, median=median_val, min=min_val, max=max_val);

plot_val <- ggplot(values, aes(x=id, y=mean, ymin=min, ymax=max));
plot_val <- plot_val + geom_step();
plot_val <- plot_val + geom_step(aes(y=min)) + geom_step(aes(y=max)); # with min & max
