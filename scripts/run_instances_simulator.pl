...
sub run_schedule {
	...
	my $schedule_result = `$schedule_script $cluster_size $backfilling_variant $delay $socket_file $json_file`;
...
sub write_results_to_file {
	...
	#TODO Use join to have spaces between them
	my @headers;
	for my $comm_level ("CL", "CM", "CH") {
		for my $backfilling_variant (@backfilling_variants) {
			push @headers, map { $comm_level . '_' . $BACKFILLING_VARIANT_STRINGS[$backfilling_variant] . '_' . $_ } ("CMAX", "CJOBS", "LJOBS", "LFACTOR", "RTIME");
		}
	}
