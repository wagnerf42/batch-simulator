...
\Function{remove\_task}{st\_t, d\_t, cpu\_t}
	...
	\State
	\For{tous les emplacements restant}
		\State $tab\_resultat = tab\_resultat\ \cup$ \textbf{extend\_freespace}$(emplacement)$
	\EndFor
	\State
	\State $tab\_final \gets $ vide
	\State
...
\Function{extend\_freespace}{$freespace, task$}
	...
	\State
	\If{(temps de départ de $freespace$ = temps final de $task$) \textbf{or}(temps de départ de $task$ = temps final de $freespace$)}
		\If{les cpu de $freespace$ et $task$ sont les mêmes}
			\State On modifie l'emplacement $freespace$ avec :
			\State \hspace{\algorithmicindent} Temps de départ : temps de départ \textbf{min}($task, freespace$)
			\State \hspace{\algorithmicindent} Durée: Durée de $task$ + durée de $freespace$
			\State \hspace{\algorithmicindent} Cpu : Cpu de $freespace$
		\State $new\_emplacements \gets new\_emplacements\ \cup\ freespace$  
		\Else
			\State On crée un nouvel emplacement $new\_freespace$ avec :
			\State \hspace{\algorithmicindent} Temps de départ : temps de départ \textbf{min}($task, freespace$)
			\State \hspace{\algorithmicindent} Durée: Durée de $task$ + durée de $freespace$
			\State \hspace{\algorithmicindent} Cpu : Cpu de $freespace$ $\cap$ cpu de $task$
		\State $new\_emplacements \gets new\_emplacements\ \cup\ new\_freespace \cup\ freespace$
		\EndIf
	\Else
		\State On crée un nouvel emplacement $new\_freespace$ avec :
			\State \hspace{\algorithmicindent} Temps de départ : temps de départ \textbf{max}($freespace, task$)
			\State \hspace{\algorithmicindent} Durée: temps de départ \textbf{max}($freespace, task$) - temps final \textbf{min}($freespace, task$)
