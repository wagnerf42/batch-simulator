...
\Function{extend\_freespace}{$freespaces$}
	...
	\State
	\For{chaque éléments de $freespaces$}
	...
	\State
	\For{chaque éléments de $liste\_events$}
		\State $temps \gets$ temps de élément
		\If{éléments est de type "start"}
			\State $cpu \gets cpu\ \cup$ cpu de élément
		\ElsIf{éléments est de type "end"}
			\State $cpu \gets cpu\ -$ cpu de élément
		\EndIf
		\If{$cpu\_init\ \neq\ cpu\ \textbf{and}\ temps\_init\ \neq\ temps$}			\State On crée un nouvel emplacement $new\_freespace$ avec :
