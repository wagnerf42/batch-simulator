...
\Function{cut\_freespace}{$freespace, start\_time, duration, processor\_range$}
	\If{($freespace\rightarrow\{cpu\}\ \textbf{inter}\ processor\_range) > 0$}
...
\Function{remove\_task}{st\_t, d\_t, cpu\_t}
	...
	\State
	\For{$t\gets st\_t\ \textbf{to}\ et\_t$}
		\Comment{Update cpu range for the job placement}
	...
	\State $profile\_tree\rightarrow \textbf{node\_loop}(start\_key, end\_key, \{$
		\State
		\If{$(cpu\_t\ \textbf{inter}\ freespace\{cpu\}) > 0$}
			...
			\If{$\textbf{exist}\ direction\{freespace\}$}
				\For{$t\gets st\_t2\ \textbf{to}\ et\_t2$}
					\Comment{Update cpu range for freespace placement}
					\State					
...
\begin{algorithm}
\caption{Suppression des Freespaces inutiles}\label{remove_unnecessary_space}
\begin{algorithmic}[1]
\Function{remove\_unnecessary\_freespace}{$freespace, freespace\_list$}
	\For{$space \gets freespace\_list$}
		\If{$(freespace\rightarrow\{cpu\}\ \textbf{inter}\ space\rightarrow\{cpu\}) = (freespace\rightarrow\{cpu\}\rightarrow\textbf{size})$}
			\If{$((freespace\rightarrow\{starting\_time\} \leq  space\rightarrow\{starting\_time\})\ \textbf{and}\ (freespace\rightarrow\{starting\_time\} + freespace\rightarrow\{duration\} \leq space\rightarrow\{starting\_time\} + space\rightarrow\{duration\}))$}
				\State $profile\_tree\rightarrow \textbf{remove}(freespace)$
				\State \textbf{return}
			\EndIf
		\EndIf
